...
pytest tests/ -v                     # run strategy tests (349 tests)
...
- **2 LLM strategies**: `llm_signal_generator` (basic Azure OpenAI, full order type control), `llm_autonomous_trader` (thesis-driven 15-min multi-timeframe with reflective learning — see below).
...
- LLM strategies subclass `LLMStrategy` from `llm_base.py`, not `Strategy` directly. Use `max_completion_tokens` (not `max_tokens`) for newer Azure models. Exception: `llm_autonomous_trader` subclasses `Strategy` directly and uses `AzureOpenAIClient` + `PositionManager` for full control.
