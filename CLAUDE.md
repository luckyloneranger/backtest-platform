...
- `strategies/narrative_builder.py` — Translates raw indicator values into qualitative English narratives for LLM consumption. 4 functions: `build_symbol_narrative` (per-stock analysis with confluence scoring), `build_regime_narrative` (ADX/BBW market state), `build_portfolio_narrative` (drawdown/costs), `build_trade_history_narrative` (win rate, lessons from losses).
