...
pytest tests/ -v                     # run strategy tests (283 tests)
...
- **3 ML-powered strategies**: `ml_classifier` (GradientBoosting with 20+ features, rolling training), `ou_mean_reversion` (Ornstein-Uhlenbeck process via statsmodels — **consistently profitable**), `ensemble_meta` (LogisticRegression meta-learner on 5 sub-signals).
...
- **Capital-dependent behavior**: At ₹1L capital, Portfolio Combiner (+23%/+293%) works well; most other strategies lose due to position sizing constraints. At ₹10L, more strategies become viable. Strategy defaults are optimized for ₹1L-₹10L range.
- **Parallel execution**: `run_all_strategies.sh` runs all strategies in parallel via multiple gRPC servers. `sweep.sh` runs parameter sweeps with isolated server instances per backtest. Server supports `--port` for multi-instance.
